...
def next_ada_clip_bound(beta, norm_bound,gamma):
      ...
      if decay_policy == 'Linear':
          ...
          grad_clip = empirical_fraction-target_unclipped_quantile
          grad_clip = tf.exp(-learnng_rate*grad_clip)
...
def compute_beta(g, l2_norm_clip):
  ...
  return tf.cond(global_norm < l2_norm_clip, lambda:tf.constant(1.0,dtype=tf.float32), lambda: tf.constant(0.0,dtype=tf.float32))
...
def make_vectorized_keras_optimizer_class(cls):
  ...
  class DPOptimizerClass(cls):
    ...
    def __init__(
      ...
      self._l2_norm_clip = tf.Variable(l2_norm_clip)
    ...
    def _compute_gradients(self, loss, var_list, grad_loss=None, tape=None):
      ...
      with tf.keras.backend.name_scope(self._name + '/gradients'):
        ...
        beta_micobatch = tf.vectorized_map(
            lambda g: compute_beta(g, self._l2_norm_clip), jacobian)
        tf.print(beta_micobatch)
        beta = tf.reduce_mean(beta_micobatch)
        self._l2_norm_clip.assign(next_ada_clip_bound(beta, self._l2_norm_clip, self._gamma))
      ...
      return final_gradients#list(zip(final_gradients, var_list))
